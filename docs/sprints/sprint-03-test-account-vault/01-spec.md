...
- **Env-level permission** — kiểm soát member chỉ xem được môi trường nào (dev/staging/production) → Sprint 04
...
|--------|:-----------:|:---------------------:|:---------------------:|:----------:|
| List / Get | ✅ all envs | ✅ chỉ envs trong `allowedEnvs`* | ✅ chỉ envs trong `allowedEnvs`* | ❌ 403 |
| Create | ✅ any env | ✅ trong `allowedEnvs` của mình | ❌ 403 | ❌ 403 |
| Update | ✅ any env | ✅ trong `allowedEnvs` của mình | ❌ 403 | ❌ 403 |
| Delete | ✅ any env | ✅ trong `allowedEnvs` của mình | ❌ 403 | ❌ 403 |

> "Admin (project member)" = user có system role `admin` (hoặc cao hơn) VÀ đang là active member của project đó.
>
> \* `allowedEnvs` — danh sách môi trường được phép, gán khi add member vào project (mặc định `['dev']`). Thay đổi bởi SA hoặc Admin của project. **Chi tiết implement ở Sprint 04** — Sprint 03 chưa có cột này; tạm thời member thấy cả 3 env (behavior hiện tại giữ nguyên cho đến khi Sprint 04 deploy).
