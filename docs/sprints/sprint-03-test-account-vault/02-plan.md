...
| D11 | Member có xem được account của **cả 3 môi trường** không? | **Không** — xem được môi trường nào phụ thuộc `allowedEnvs` được gán khi add vào project | Production nhạy cảm hơn dev; cần kiểm soát rõ ai được phép xem. SA luôn thấy tất cả. Chi tiết design ở Sprint 04. |
...
- **Env-level permission** (`allowedEnvs` per member) — thiết kế đầy đủ và implement ở Sprint 04
